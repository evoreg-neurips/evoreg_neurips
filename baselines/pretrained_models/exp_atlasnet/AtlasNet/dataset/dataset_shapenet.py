...
import baselines.pretrained_models.exp_atlasnet.AtlasNet.dataset.pointcloud_processor as pointcloud_processor
...
class ShapeNet(data.Dataset):
    ...
    def __init__(self, opt, train=True):
        ...
        if not opt.demo:
            ...
            # Load classes
            self.pointcloud_path = join(opt.dir, 'ShapeNetV1PointCloud')
            self.image_path = join(opt.dir, 'ShapeNetV1Renderings')

            # Load taxonomy file
            self.taxonomy_path = join(opt.dir, 'taxonomy.json')
            if not exists(self.taxonomy_path):
                ...
                raise FileNotFoundError(f"{self.taxonomy_path} not found. Please provide valid data path")
            ...
            # Create Cache path
            self.path_dataset = join(opt.dir, 'cache')
            ...
            if not exists(self.image_path):
                ...
                raise FileNotFoundError(f"{self.image_path} not found. Please provide valid data path")
